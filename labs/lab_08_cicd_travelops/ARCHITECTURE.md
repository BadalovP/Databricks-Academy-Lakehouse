...
- **Azure PROD**:
  ...
  - `run_lakeflow_pipeline` pipeline task: executes the Lakeflow pipeline using **classic pipeline-managed compute** (`serverless: false`, `photon: false`, single-label `default` cluster with `Standard_F4` nodes, `ON_DEMAND_AZURE`, and enhanced autoscaling `min_workers: 1`, `max_workers: 2`). `resources/pipeline.yml` defaults to `photon: true`; the Azure PROD target explicitly overrides `photon: false` because `Standard_F4` does not support Photon as a worker node type (PR #13). Personal DEV and Personal PROD do not set this override and keep `photon: true` on serverless compute.
...
PAT and OIDC deployments share the pinned Azure bundle root `/Workspace/Users/parvinbadalov@softserve.academy/.bundle/lab08-travelops-cicd/azure_prod`. OIDC is the active deployment path. The existing human user retains `IS_OWNER` on the job and pipeline, while the GitHub service principal has resource-specific `CAN_MANAGE` permissions. The dashboard and alert also have resource-specific service-principal management grants. The shared SQL warehouse provides `CAN_USE`. No Azure target-level permission propagation is used, preventing unintended ownership reconciliation.

The September 18 deployment successfully applied the GP2/classic-compute configuration. Final bundle validation reported zero changes. The first manual execution of the production job under this configuration failed at pipeline cluster creation because `Standard_F4` does not support Photon, which `resources/pipeline.yml` enables by default; PR #13 scoped `photon: false` to the Azure PROD pipeline override only. A subsequent manual retry of job run `388990177883700` succeeded on the second attempt of `run_lakeflow_pipeline`, and all three promotion job tasks completed with an overall `SUCCESS` result. Gold health validation passed (`health_passed = true`), though a separate, unresolved payment-reconciliation observation was found in that run's output — see `README.md` for the investigation. Azure PROD execution remains manual: `LAB08_RUN_AZURE_PROD_JOB=false` keeps GitHub Actions from running the job automatically.
