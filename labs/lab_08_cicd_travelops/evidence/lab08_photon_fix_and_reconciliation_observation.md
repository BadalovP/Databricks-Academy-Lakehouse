...
## Status update (added when this file was ported into the evidence directory)

This file was originally written on 2026-09-19 as part of PR #14 and records
a point-in-time, read-only investigation of Azure PROD. It is preserved here
verbatim below, unedited, as the original root-cause record for the 100%
payment-mismatch finding it documents. **The defects it describes have since
been fixed and independently verified**, and the observations below should
be read as historical evidence of the problem, not as the current state of
the system:

- The "always true" health gate (`payment_mismatch_count >= 0`) described
  below was replaced by a genuine 5-state reconciliation model and a real
  `health_passed` gate in PR #15. See
  `evidence/lab08_personal_dev_run1_after_results.md` and
  `evidence/lab08_personal_dev_run2_after_results.md` for the gate correctly
  failing on a real defect and then correctly passing once that defect was
  fixed.
- The raw-ingestion idempotency defect (Auto Loader re-ingesting the same
  deterministic seed as new files on every run, inflating Bronze roughly
  7x here) was fixed by the immutable, write-once seed design; see
  `evidence/lab08_production_remediation_plan.md` for the fix and the
  still-open historical-cleanup decision, and this note's next point for the
  fix's own confirmation on Azure PROD.
- The prediction in the original text below, that "Personal DEV and
  Personal PROD share the same Bronze/Silver code... the same defect should
  be assumed possible there until checked," was later checked and
  confirmed: Personal DEV independently exhibited its own historical
  duplication (a different factor, ~16x rather than ~7x) before the fix,
  documented in `evidence/lab08_personal_dev_before_baseline.md`.
- Azure PROD's own execution of the fixed code is documented in
  `evidence/lab08_azure_prod_run_989280124372132.md`. **Historical Bronze
  duplication accumulated before the fix — including the 7x figures
  recorded below — has not been retroactively cleaned up anywhere**; the
  scoped, human-approved full-refresh remediation in
  `evidence/lab08_production_remediation_plan.md` remains written but not
  executed.
