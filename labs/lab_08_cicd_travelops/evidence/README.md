...
| Personal DEV BEFORE baseline | Personal DEV | Captured, read-only | `lab08_personal_dev_before_baseline.md` records Bronze row counts, Silver duplicate-key group counts, Gold reconciliation/health and revenue totals for the currently-deployed (pre-PR-15) code, for comparison against the AFTER state once `fix/lab08-payment-idempotency-reconciliation` is deployed and run. |
| Personal DEV Run 1 (first controlled integration test) | Personal DEV | Deploy/seed/pipeline succeeded; health gate correctly FAILED on a real defect | `lab08_personal_dev_run1_after_results.md` records job run `922281283343187`, pipeline update `a765e63d-7e92-419a-a457-63dde2c1c9ad`, the new seed files, and a full BEFORE/AFTER comparison confirming every dedup and join-cardinality fix works correctly against live 16x-duplicated Bronze data. `health_passed=false` was originally attributed to a genuine source-data underpayment for booking_id 13594; a follow-up investigation found the true cause was a `current_bookings_silver` ordering defect (see the file's correction note and the Run 2 row below), fixed in commit `ab9a78d`. |
| Personal DEV Run 2 (second controlled integration test) | Personal DEV | Success; health gate now correctly passes | `lab08_personal_dev_run2_after_results.md` records job run `366859582717065`, pipeline update `161fab6c-5639-41ad-943b-bc364a1548cb`. Confirms the `current_bookings_silver` ordering fix: booking_id 13594 now selects $775.56/confirmed and reconciles as `matched`; `payment_amount_mismatch_count=0`, `health_passed=true`. Also confirms the rerun is a true no-op at the raw-file and Bronze level (identical filenames and row counts to Run 1) and that Gold property/destination revenue remain exactly equal to the independent Silver source total. |
| Azure PROD job run (first execution of fixed code) | Azure PROD | Databricks job SUCCESS (all 3 tasks); GitHub Actions monitoring failed separately | `lab08_azure_prod_run_989280124372132.md` records job run `989280124372132`, pipeline update `be3996b6-9e42-452f-aca1-c01c230e24fe` (correlated by task time window, not assumed), and diagnoses why the triggering GitHub Actions run (`35474831164`) failed to locate/poll the run afterward: a standalone Databricks CLI command inside the bundle directory resolved the Personal DEV workspace host instead of Azure PROD, a workflow defect distinct from the job itself. `health_passed`/`payment_amount_mismatch_count` were not independently re-queried, since the SQL warehouse was stopped and was not started to check. |
