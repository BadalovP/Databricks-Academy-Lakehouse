...
| Personal DEV Run 1 (first controlled integration test) | Personal DEV | Deploy/seed/pipeline succeeded; health gate FAILED on one genuine $43 underpayment | `lab08_personal_dev_run1_after_results.md` records job run `922281283343187`, pipeline update `a765e63d-7e92-419a-a457-63dde2c1c9ad`, the new seed files, and a full BEFORE/AFTER comparison confirming every dedup and join-cardinality fix works correctly against live 16x-duplicated Bronze data; `health_passed=false` reflects the gate correctly catching booking_id 13594's real underpayment, not a pipeline defect. |
