...
def classified():
    return classify_license_records(
        prepare_business_licenses(spark.read.table("business_license_bronze"))
    )


@dp.materialized_view(name="business_license_quarantine")
def quarantine():
    return spark.read.table("business_license_classified").filter(
        "size(_dq_quarantine_reasons) > 0"
    )
...
@dp.expect(
    "dba_name_present",
    "doing_business_as_name IS NOT NULL",
)
@dp.expect_or_fail(
...
def validated():
    return spark.read.table("business_license_classified")
